...
class RejectedLesson extends StatelessWidget {
  List<Data> data;
   RejectedLesson({Key? key,required this.data}) : super(key: key);
            ...
            onTap: () {
              TeacherController().returnTeacherClassList().then((classList) {
                TeacherController().teacherClassHomeWork(
                    ...
                    navigate: false,
                    classSectionMasterid: data[i].sectionMasterId.toString(),
                    className: data[i].className.toString()).then((value){
                  Get.to(() => CreateLesson(
                    data: data[i],
                    notEditable: true,
                    subjectList: value,
